...
// Exported (not just used internally) so admin/username-fixer.ts can both
// recognize old auto-generated names and generate fresh suggestions for them.
...
export function generateFriendlyUsernameBase(): string {
...
// True if `username` still looks like it was never touched since being
// auto-generated — either the legacy "Guest_xxxxxx"/"Guest####" format, or
// the current "AdjectiveNoun##" format (still starts with one of our
// adjectives). Someone who's since renamed themselves to something unrelated
// won't match this anymore.
export function isAutoGeneratedUsername(username: string): boolean {
  if (/^Guest[_\d]/i.test(username)) return true;
  const lower = username.toLowerCase();
  return USERNAME_ADJECTIVES.some((adj) => lower.startsWith(adj.toLowerCase()));
}
...
export class Auth {
  ...
  async autoRegister(): Promise<AuthResult & { username?: string; password?: string }> {
    ...
    const username = await this.generateUsername(generateFriendlyUsernameBase());

...
import { generateFriendlyUsernameBase, isAutoGeneratedUsername, OWNER_USER_ID } from '../../lib/auth';
...
async function suggestClean(db: any, name: string, id: number): Promise<string> {
  ...
  let final = clean;
  let i = 2;
  while (await db.fetchOne('SELECT id FROM users WHERE LOWER(username)=LOWER(?) AND id!=?', [final, id])) {
    final = clean + i++;
  }
  return final;
}

// Same idea as suggestClean, but for auto-generated ("Guest4821",
// "SwiftRonin82") names that are already *valid* format — a random
// adjective+noun handle is just a nicer replacement than a cleaned-up
// version of the same generated string.
async function suggestFriendly(db: any, id: number): Promise<string> {
  let name = generateFriendlyUsernameBase();
  while (await db.fetchOne('SELECT id FROM users WHERE LOWER(username)=LOWER(?) AND id!=?', [name, id])) {
    name = generateFriendlyUsernameBase();
  }
  return name;
}

// autoRegister() always logs this exact action+details pair, so it doubles
// as a reliable "was this account auto-created?" marker — much sturdier
// than guessing from the username alone, and it survives the account being
// later renamed to something that no longer looks auto-generated.
async function getAutoCreatedIds(db: Db): Promise<Set<number>> {
  const rows = await db.fetchAll<{ user_id: number }>(
    "SELECT DISTINCT user_id FROM activity_log WHERE action='register' AND details LIKE 'Auto-created%'"
  );
  return new Set(rows.map((r) => r.user_id));
}

// Normally every account auto-follows the Owner at registration time (see
// register()/autoRegister() in auth.ts), but accounts created on
// beta.anivault.co (or any other path that inserts into `users` without
// going through those flows) can slip through without that follow row.
// This just finds anyone missing it — no need to touch the registration
// code itself, this page is a net that catches whatever falls through.
async function getUsersNotFollowingOwner(db: Db): Promise<Set<number>> {
  const rows = await db.fetchAll<{ id: number }>(
...
adminUsernameFixerRoutes.on(['GET', 'POST'], '/admin/username_fixer.php', async (c) => {
  ...
  if (c.req.method === 'POST' && isOwner) {
    ...
    if (action === 'fix_one') {
      ...
      if (uid && /^[a-zA-Z0-9_]{3,30}$/.test(newName)) {
        const conflict = await db.fetchOne('SELECT id FROM users WHERE LOWER(username)=LOWER(?) AND id!=?', [newName, uid]);
        if (conflict) {
    ...
    } else if (action === 'follow_one') {
      const uid = parseInt((body.user_id as string) ?? '0', 10) || 0;
      if (uid && uid !== OWNER_USER_ID) {
        await db.query('INSERT OR IGNORE INTO follows (follower_id, following_id) VALUES (?, ?)', [uid, OWNER_USER_ID]);
        await Logger.log(db, userId, 'owner_fix_follow', `Made user ${uid} follow the Owner`);
        session.setFlash('success', `User #${uid} now follows the Owner.`);
      }
    } else if (action === 'auto_fix_all') {
      const allUsers = await db.fetchAll<{ id: number; username: string }>('SELECT id, username FROM users');
      const autoCreatedIds = await getAutoCreatedIds(db);
      const notFollowingIds = await getUsersNotFollowingOwner(db);
      const bad = allUsers.filter((u) => isBadUsername(u.username));
      const guests = allUsers.filter((u) => !isBadUsername(u.username) && autoCreatedIds.has(u.id) && isAutoGeneratedUsername(u.username));
      let fixed = 0;
      for (const u of bad) {
      ...
      for (const u of guests) {
        const final = await suggestFriendly(db, u.id);
        await db.query('UPDATE users SET username=? WHERE id=?', [final, u.id]);
        await Logger.log(db, userId, 'owner_fix_username', `Auto-fixed guest ${u.id}: '${u.username}' → '${final}'`);
        fixed++;
      }
      for (const uid of notFollowingIds) {
        await db.query('INSERT OR IGNORE INTO follows (follower_id, following_id) VALUES (?, ?)', [uid, OWNER_USER_ID]);
        await Logger.log(db, userId, 'owner_fix_follow', `Made user ${uid} follow the Owner`);
        fixed++;
      }
      session.setFlash('success', `Auto-fixed ${fixed} issue(s).`);
    }
  ...
  const guestUsers = allUsers.filter((u) => !isBadUsername(u.username) && autoCreatedIds.has(u.id) && isAutoGeneratedUsername(u.username));
  const notFollowingUsers = allUsers.filter((u) => notFollowingIds.has(u.id));
  const flagged = [
    ...badUsers.map((u) => ({ ...u, reason: 'invalid' as const })),
    ...guestUsers.map((u) => ({ ...u, reason: 'auto' as const })),
    ...notFollowingUsers.map((u) => ({ ...u, reason: 'not_following' as const })),
  ];
  const suggestions: Record<number, string> = {};
  if (isOwner) {
    for (const u of badUsers) suggestions[u.id] = await suggestClean(db, u.username, u.id);
    for (const u of guestUsers) suggestions[u.id] = await suggestFriendly(db, u.id);
  }
  ...
  html += `
<div class="admin-header"><h1>🔧 Username Fixer</h1><span class="text-muted">Scan and fix invalid/auto-generated usernames and accounts missing the auto-follow of the Owner</span></div>
${err ? `<div class="alert alert-error mb-2">⚠️ ${h(err)}</div>` : ''}
${suc ? `<div class="alert alert-success mb-2">✅ ${h(suc)}</div>` : ''}

${flagged.length === 0 ? `
<div class="card card-body text-center" style="padding:3rem;"><div style="font-size:3rem;margin-bottom:1rem;">✅</div><h2>All clean!</h2><p class="text-muted">No accounts with invalid characters, untouched auto-generated names, or a missing follow of the Owner were found.</p></div>` : `
<div class="alert alert-error mb-3" style="display:flex;align-items:center;gap:1rem;justify-content:space-between;flex-wrap:wrap;">
  <span>⚠️ Found <strong>${flagged.length}</strong> account(s): <strong>${badUsers.length}</strong> with invalid usernames, <strong>${guestUsers.length}</strong> still on an auto-generated guest name, <strong>${notFollowingUsers.length}</strong> not following the Owner.</span>
  ${isOwner ? `<form method="POST" onsubmit="return confirm('Auto-fix all ${flagged.length} issues? Invalid usernames get cleaned up, auto-generated guest names get a fresh random handle, and accounts missing the Owner follow will start following.')"><input type="hidden" name="action" value="auto_fix_all"><button type="submit" class="btn btn-primary btn-sm">⚡ Auto-Fix All</button></form>` : ''}
</div>

<div class="card" style="overflow-x:auto;">
  <div class="data-table-wrap"><table class="data-table">
    <thead><tr><th>ID</th><th>Current Username</th><th>Reason</th><th>Email</th><th>Joined</th>${isOwner ? '<th>Suggested Fix</th><th>Action</th>' : ''}</tr></thead>
    <tbody>
      ${flagged.map((u) => `
      <tr>
        ...
        <td>${u.reason === 'invalid'
          ? `<span class="badge" style="background:rgba(255,42,42,0.15);color:#ff6b6b;">Invalid format</span>`
          : u.reason === 'auto'
          ? `<span class="badge" style="background:rgba(255,193,7,0.15);color:#ffc107;">Auto-generated</span>`
          : `<span class="badge" style="background:rgba(100,149,237,0.15);color:#7aa2f7;">Not following Owner</span>`}</td>
        ...
        ${isOwner ? (u.reason === 'not_following' ? `
        <td class="text-muted">—</td>
        <td><form method="POST" style="display:inline;"><input type="hidden" name="action" value="follow_one"><input type="hidden" name="user_id" value="${u.id}"><button type="submit" class="btn btn-ghost btn-sm">➕ Follow</button></form></td>` : `

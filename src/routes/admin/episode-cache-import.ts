 ...
 * finished show is marked as never needing to check again at all.
 *
 * Per-episode rows are written via db.batch() in chunks, NOT one
 * putCachedRaw() await per episode -- a 1000+ episode show (One Piece,
 * Naruto Shippuden) doing 1000+ sequential D1 round trips inside one
 * request blows past Workers' per-request subrequest limit and the whole
 * request dies with a generic network error before ever responding.
 * db.batch() sends a whole chunk as ONE D1 call, so IMPORT_BATCH_SIZE
 * episodes cost 1 subrequest instead of IMPORT_BATCH_SIZE of them --
 * collapses a 1000+ episode import down to single-digit subrequests total.
 */
...
export async function importAnimeEpisodeThumbnails(
  ...
  const validEntries: Array<{ epNum: number; thumb: string }> = [];
  let skipped = 0;
  ...
  for (const ep of episodes) {
    ...
    bulk[epNum] = thumb;
    validEntries.push({ epNum, thumb });
  }

  const upsertSql = `INSERT INTO episode_thumb_cache (cache_key, value, expires_at, created_at)
  ...
  for (let i = 0; i < validEntries.length; i += IMPORT_BATCH_SIZE) {
    const chunk = validEntries.slice(i, i + IMPORT_BATCH_SIZE);
    const statements = chunk.map(({ epNum, thumb }) =>
      stmt.bind(episodeThumbCacheKey(malId, epNum), JSON.stringify({ success: true, thumb }))
    );
    try {
      await db.batch(statements);
    } catch (err: any) {
      console.warn(`[episode-thumb] import batch ${i}-${i + chunk.length} failed (continuing with remaining chunks):`, String(err?.message ?? err));
    }
  }
  ...
  return { malId, imported: validEntries.length, skipped, permanent };
}
